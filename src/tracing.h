...
inline std::string get_formatted_time() {
    ...
    const auto now_str = std::string(buffer) + "." +
                         std::string(3 - ms_str.size(), '0') + ms_str;

    return now_str;
}

template <typename... T>
void trace(const std::string &level, fmt::format_string<T...> fmt,
           T &&...args) {
...
template <typename... T> void info(fmt::format_string<T...> fmt, T &&...args) {
